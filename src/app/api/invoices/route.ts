...
function sanitizeSearchString(searchText: string): string {
  ...
  // استخدام صيغة أكثر أمانًا للتعامل مع الأحرف العربية
  return searchText.trim().replace(/[%_\\]/g, ""); // إزالة الرموز الخاصة في MySQL LIKE
}

// GET - استرجاع قائمة الفواتير
export async function GET(request: NextRequest) {
  try {
    ...
    // التحقق من الصلاحيات (يمكن للمدير والمحاسب فقط الوصول)    if (      payload.role !== "ADMIN" &&      payload.role !== "ACCOUNTANT"    ) {      return NextResponse.json(        { message: "ليس لديك صلاحية للوصول إلى هذا المورد" },        { status: 403 }      );    }
    ...
    // استرجاع الفواتير مع العدد الإجمالي
    ...
    let errorMessage = "حدث خطأ أثناء استرجاع بيانات الفواتير";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // التحقق من الصلاحيات (يمكن للمدير والمحاسب فقط الإضافة)    if (      payload.role !== "ADMIN" &&      payload.role !== "ACCOUNTANT"    ) {      return NextResponse.json(        { message: "ليس لديك صلاحية لإضافة فاتورة جديدة" },        { status: 403 }      );    }
    ...
    if (
      ...
      return NextResponse.json(
        { message: "معرف المريض والمبلغ الإجمالي وعناصر الفاتورة حقول مطلوبة" },
        { status: 400 }
      );
    }
